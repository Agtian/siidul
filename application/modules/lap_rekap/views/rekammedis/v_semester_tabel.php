        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <br>
                        <?php echo form_open('lap_rekap/semester', 'class="form-horizontal "'); ?>
                            ...
                            <div class="form-group">
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a class="btn btn-primary" target="_blank" href="<?php echo base_url('lap_rekap/export_semester/').$id_ruang_sub.'/'.$tahun; ?>">Export PDF</a>
                                </div>
                            </div>
                        <?php echo form_close(); ?>
                    </div>
                                                ...
                                                <?php 
                                                    if ($no == '4' || $no == '5')
                                                    {
                                                        if (!empty($NUM_JUL) && empty($row->NUM_AGT) && empty($NUM_SEP) && empty($row->NUM_OKT) && empty($row->NUM_NOV) && empty($NUM_DES)) {
                                                            ...
                                                            $persen = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt);
                                                            echo round($persen, 5);

                                                        } else if (!empty($NUM_JUL) && !empty($row->NUM_AGT) && !empty($NUM_SEP) && !empty($row->NUM_OKT) && !empty($row->NUM_NOV) && empty($NUM_DES)) {
                                                            
                                                            $persen = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt) + ($row->NUM_NOV / $tt_hari_nov);
                                                            echo round($persen, 5);

                                                        } else if (!empty($NUM_JUL) && !empty($row->NUM_AGT) && !empty($NUM_SEP) && !empty($row->NUM_OKT) && !empty($row->NUM_NOV) && !empty($NUM_DES)) {
                                                            
                                                            $persen = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt) + ($row->NUM_NOV / $tt_hari_nov) + ($row->NUM_DES / $tt_hari_des);
                                                            echo round($persen, 5);
                                                        } else if (empty($NUM_JUL) && empty($row->NUM_AGT) && empty($NUM_SEP) && empty($row->NUM_OKT) && empty($row->NUM_NOV) && empty($NUM_DES)) {
                                                ...
                                                <?php
                                                    if ($no == '4' || $no == '5')
                                                    {
                                                        if (!empty($NUM_JUL) && empty($row->NUM_AGT) && empty($NUM_SEP) && empty($row->NUM_OKT) && empty($row->NUM_NOV) && empty($NUM_DES)) {
                                                            ...
                                                            $num    = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt);
                                                            ...
                                                            $num    = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt);
                                                            $den    = $row->DEN_JUL + $row->DEN_AGT + $row->DEN_SEP + $row->DEN_OKT;
                                                            $persen = $num / $den;
                                                            echo gmdate('H:i:s', floor($persen * 86400));

                                                        } else if (!empty($NUM_JUL) && !empty($row->NUM_AGT) && !empty($NUM_SEP) && !empty($row->NUM_OKT) && !empty($row->NUM_NOV) && empty($NUM_DES)) {

                                                            $num    = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt) + ($row->NUM_NOV / $tt_hari_nov);
                                                            $den    = $row->DEN_JUL + $row->DEN_AGT + $row->DEN_SEP + $row->DEN_OKT + $row->DEN_NOV;
                                                            $persen = $num / $den;
                                                            echo gmdate('H:i:s', floor($persen * 86400));

                                                        } else if (!empty($NUM_JUL) && !empty($row->NUM_AGT) && !empty($NUM_SEP) && !empty($row->NUM_OKT) && !empty($row->NUM_NOV) && !empty($NUM_DES)) {

                                                            $num    = ($row->NUM_JUL / $tt_hari_jul) + ($row->NUM_AGT / $tt_hari_agt) + ($row->NUM_SEP / $tt_hari_sep) + ($row->NUM_OKT / $tt_hari_okt) + ($row->NUM_NOV / $tt_hari_nov) + ($row->NUM_DES / $tt_hari_des);
                                                            $den    = $row->DEN_JUL + $row->DEN_AGT + $row->DEN_SEP + $row->DEN_OKT + $row->DEN_NOV + $row->DEN_DES;
                                                            $persen = $num / $den;
                                                            echo gmdate('H:i:s', floor($persen * 86400));
