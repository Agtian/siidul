                                        ...
                                        <td align="center">
                                            <?php
                                                if ($row->NUM_JAN == 0)
                                                {
                                                    ...
                                                    $total = $row->NUM_JAN / $tt_hari_jan;
                                                    echo gmdate('H:i:s', floor($total * 86400));
                                                } else {
                                                    echo $row->NUM_JAN;
                                                }
                                            ?> 
                                                ...
                                                <?php 
                                                    if ($no == '2')
                                                    {
                                                        if ($row->TOTAL_NUM_TW_I == 0) 
                                                        {
                                                            ...
                                                            // $total = $num / 9;
                                                            // echo gmdate('H:i:s', floor($total * 86400));
                                                            //$num    = $row->NUM_JAN + $row->NUM_FEB + $row->NUM_MAR + $row->NUM_APR + $row->NUM_MEI + $row->NUM_JUN + $row->NUM_JUL + $row->NUM_AGT + $row->NUM_SEP;
                                                            $den     = $row->DEN_JAN + $row->DEN_FEB + $row->DEN_MAR + $row->DEN_APR + $row->DEN_MEI + $row->DEN_JUN  + $row->DEN_JUL + $row->DEN_AGT + $row->DEN_SEP;
                                                            // $total = ($num / 9) / 43800;
                                                            //$persen = (($num / $tt_den) / 9) / 43800 / $tt_den;
                                                            // echo $total;
                                                            // .gmdate('H:i:s', floor($persen * 86400))
                                                            echo gmdate('H:i:s', floor($num * 1440));
                                                        }
                                                    } else {
                                                        if ($row->TOTAL_NUM_TW_I == 0) 
                                                        {
